...
void ABlasterPlayerController::HighPingWarning()
{
	...
	if (bHUDValid) {
		...
		BlasterHUD->CharacterOverlay->PlayAnimation(BlasterHUD->CharacterOverlay->HighPingAnimation, 0.f, 3);
	}
}
...
//when gamemode call boradcastElim it calls clientelimannouncement
void ABlasterPlayerController::ClientElimAnnouncement_Implementation(APlayerState* Attacker, APlayerState* Victim)
{
	...
	if (Attacker && Victim && Self)
	{
		...
		if (BlasterHUD)
		{
			...
			{
				BlasterHUD->AddElimAnnouncement(Attacker->GetPlayerName(), "themseleves");
				return;
			}
...
// override function이라 클라 켜지자마자 실행임.
		...
		if (bHUDValid) {
			...
			if (BlasterGameState && BlasterPlayerState) {
				...
				if (TopPlayers.Num() == 0) {
					...
					InfoTextString = FString("Tied Players:\n");
